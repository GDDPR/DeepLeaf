...
# Safer for small local models (ex: 4k-context Mistral served by vLLM)
MAX_PROMPT_CHARS = int(os.getenv("PAGEINDEX_MAX_PROMPT_CHARS", "12000"))
...
def _trim_prompt(prompt, max_chars=MAX_PROMPT_CHARS):
    if not prompt:
        return prompt
    if len(prompt) > max_chars:
        logging.warning(
            f"Prompt trimmed from {len(prompt)} chars to {max_chars} chars for local model safety."
        )
...
def llm_completion(model, prompt, chat_history=None, return_finish_reason=False):
    ...
    api_base = os.getenv("OPENAI_BASE_URL") or os.getenv("LITELLM_API_BASE")
...
def check_token_limit(structure, limit=110000):
    items = structure_to_list(structure)
    for node in items:
